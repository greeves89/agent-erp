...
import { FileText, Calendar, Hash, AlertTriangle } from "lucide-react";
...
const statusBadge: Record<string, { bg: string; text: string }> = {
  ...
  overdue: { bg: "bg-red-500/10 border-red-500/20", text: "text-red-400" },
  cancelled: { bg: "bg-zinc-500/10 border-zinc-500/20", text: "text-zinc-400" },
};

const typeBadge: Record<string, { bg: string; text: string; label: string }> = {
  outgoing: { bg: "bg-blue-500/10 border-blue-500/20", text: "text-blue-400", label: "Outgoing" },
  incoming: { bg: "bg-violet-500/10 border-violet-500/20", text: "text-violet-400", label: "Incoming" },
};

const statusFilters = ["all", "draft", "sent", "paid", "overdue", "cancelled"];
const typeFilters = ["all", "outgoing", "incoming"];

function isOverdue(invoice: api.ErpInvoice): boolean {
  if (invoice.status === "paid" || invoice.status === "cancelled") return false;
  return new Date(invoice.due_date) < new Date();
}

export default function InvoicesPage() {
  const [invoices, setInvoices] = useState<api.ErpInvoice[]>([]);
  ...
  const refresh = useCallback(async () => {
    try {
      const data = await api.getErpInvoices({
  ...
  return (
    <div>
      <Header title="Invoices" subtitle="Track incoming and outgoing invoices" />
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {invoices.map((invoice, i) => {
                  ...
                  const sBadge = statusBadge[effectiveStatus] ?? statusBadge.draft;
                  const tBadge = typeBadge[invoice.type] ?? typeBadge.outgoing;
                  return (
                    <motion.tr
                      key={invoice.id}
                      ...
                      className={cn(
                        "border-b border-foreground/[0.04] hover:bg-foreground/[0.02] cursor-pointer transition-colors",
